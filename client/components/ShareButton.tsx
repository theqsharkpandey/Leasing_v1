...
export default function ShareButton({ imageUrl, propertyTitle }: ShareButtonProps) {
  ...
  /**
   * WhatsApp share — uses the Web Share API (navigator.share) when available
   * to share the actual property image + link as a media message.
   * This makes WhatsApp display a full-size photo with the link in the caption.
   *
   * Falls back to the wa.me URL (text-only link) on desktop/unsupported browsers.
   */
  const shareOnWhatsApp = async () => {
    ...
    // Try Web Share API with image file (mobile browsers — Android & iOS)
    if (imageUrl && typeof navigator !== "undefined" && navigator.canShare) {
      try {
        // Fetch the property image and convert to a File object
        const response = await fetch(imageUrl);
        const blob = await response.blob();
        const ext = blob.type.includes("png") ? "png" : "jpg";
        const file = new File([blob], `property.${ext}`, { type: blob.type });

        // canShare checks if the browser supports sharing this file type
        ...
        // User cancelled or share API failed — fall through to URL fallback
        console.warn("Web Share API failed, falling back to wa.me:", err);
      }
    }

    // Fallback: open wa.me with text (desktop / unsupported browsers)
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        disabled={sharing}
        className="flex items-center gap-1.5 px-3 py-2 text-sm border border-white/10 rounded-lg text-gray-400 hover:text-white hover:border-white/20 transition-colors disabled:opacity-60"
      >
        {sharing ? (
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-52 bg-[#111118] border border-white/10 rounded-lg shadow-xl z-50 py-2">
          ...
          {/* WhatsApp — shares image + link */}
          <button
            onClick={shareOnWhatsApp}
            className="w-full text-left px-3 py-2 text-sm text-gray-400 hover:text-green-400 hover:bg-white/5 transition-colors flex items-center gap-2"
          >
